...
describe("Test claim MH feature assurance compliance checks", async function () {
  ...
  it("uses the ratio (1-in-5) check when user claiming for their first herd", async () => {
    ...
    const claimForFirstHerdSBI1 = await createSheepReviewClaim(sbi1, {
      multipleHerdFlag: true,
    });
    const claimForFirstHerdSBI2 = await createSheepReviewClaim(sbi2, {
      multipleHerdFlag: true,
    });
  ...
  it("uses ratio (1-in-5) check when claim is before the MH feature assurance start date, even if user has claimed for more than one herd", async () => {
    ...
    await createSheepReviewClaim(sbi1, { multipleHerdFlag: true });
    await createSheepReviewForAdditionalHerd(sbi1, "sh-rr-534351", "Additional herd 1");
    const sbi2 = "107346082";
    await createSheepReviewClaim(sbi2, { multipleHerdFlag: true });
    ...
    const preMhClaim1 = await createSheepReviewClaim(sbi1, {
      urn: "sh-rr-534347",
      enterVisitDateAndContinueFunc: enterPreMHReleaseDateAndContinue,
    });
    const preMhClaim2 = await createSheepReviewClaim(sbi2, {
      urn: "sh-rr-534348",
      enterVisitDateAndContinueFunc: enterPreMHReleaseDateAndContinue,
    });
